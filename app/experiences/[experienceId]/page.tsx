...
export default async function ExperiencePage({
  ...
  try {
    ...
    // Get company from experience access
    const result = await (whopSdk as any).access.checkIfUserHasAccessToExperience({ 
      userId, 
      experienceId 
    });
    
    if (result.hasAccess) {
      companyId = experienceId; // Use experienceId as companyId
      
      // Try to fetch company details
      try {
        const company = await (whopSdk as any).companies.getCompany({ id: companyId });
